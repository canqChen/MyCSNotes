...
* 代码实现，使用哈希表和链表实现，哈希表记录某个key值对应的链表节点，以常数时间复杂度操作对应链表节点。每次set或get，将对应节点移动到链表头，表示最近使用，每次淘汰将链表尾节点淘汰，表示最久未使用
  ...
  public:
      ...
      {
          if(map.count(key)){		// 已存在，更新值
              move2front(key);
              map[key]->second = val;
          }
          else{		// 不存在，插入
              history.push_front({key, val});
              map.insert({key, history.begin()});
              if(map.size()>k){		// 缓存已满，淘汰尾节点
      ...
      void move2front(int key){	// 将对应节点移动到链表头
          history.splice(history.begin(), history, map[key]);
      }
  private:
...
* 代码实现，使用两个哈希表实现。将每个访问频率的记录存储到一个链表中，每个节点记录为（key，val），一个哈希表建立访问频率到对应链表首节点的映射，另一个哈希表建立记录链表中记录的key值到该节点的映射，实现常数时间复杂度访问。每次get或set，将该记录访问频率加1，并添加到对应访问频率链表头。每次淘汰最小访问频率对应链表的尾节点
  ...
  private:
      ...
      unordered_map<int, list<node>::iterator > key2nodeit;
      unordered_map<int, list<node> > freq2node;
      int minFreq = 1; // 记录最小访问频率
  };
  ```
  ...
  string removeDuplicateLetters(string s) {
      ...
      for(auto &ch: s){
          if(visited[ch] == 0){	// 当前字母不在栈中，可能可以入栈
              while(!ret.empty() && ch < ret.back() && remainder[ret.back()] > 0){ // 栈顶元素大于当前元素(逆序)，且栈顶元素
                  visited[ret.back()] = 0;										// 剩余数大于0，则出栈。出栈将visited置0
                  ret.pop_back();
              }
...
* 代码实现：本质上求BST的任意遍历方式得到的序列集合。隐含条件：遍历了父节点才能遍历其子节点
...
* 第二步思路：
  ...
  * 假设我们原始数组是arr，得到的maxLen为`[1,2,3,1,3]`，最终输出结果为res（字典序最小的最长递增子序列），res的最后一个元素在arr中位置无庸置疑是`maxLen[i]==3`对应的下标，那么到底是`arr1[2]`还是`arr[4]`呢？如果是`arr[2]`，那么`arr[2]<arr[4]`，则`maxLen[4]=4`，与已知条件相悖。因此我们应该取`arr[4]`放在res的最后一个位置
...
* 既然我们只关心一个元素对应的**根节点**，那我们希望每个元素到根节点的路径尽可能短，最好只需要一步，形成以下效果

  <img src="https://gitee.com/canqchen/cloudimg/raw/master/img/unionfind1.jpg" alt="unionfind1" style="zoom:50%;" />
...
* 路径压缩和按秩合并如果一起使用，时间复杂度接近$\textbf{O}(n)$，但可能会破坏rank的准确性
  ...
  public:
      ...
      void merge(int node1, int node2) {
          ...
          if(rank[root1] <= rank[root2]) {
              parent[root1] = root2; // 将root1挂到root2上
          }
          else {
              parent[root2] = root1; // 将root2挂到root1上
          }
          
          if(rank[root1] == rank[root2] && root1 != root2) {
              ++rank[root2];	// root2的rank+1
          }
      }
  private:
      vector<int> parent;
